...
private val mDirs = arrayOf(
    "720", "800", "960", "1024", "1080", "1280", "1440"
)
...
private fun createdDirs() {
    try {
        for (mDir in mDirs) {
            val file = File("./lib_base/src/main/res/values-w" + mDir + "dp")
            if (file.mkdir()) generateDimenXml(mDir.toInt()) /* 创建目录后生成dimens.xml文件 */
        }
    } catch (e: Exception) {
        e.printStackTrace()
    }
...
private fun generateDimenXml(currentSize: Int) {
    ...
    try {
        ...
        for (i in 0 until MAX_DP) {
            ...
            stringBuilder.append("\t<dimen name=\"dp").append(i + 1).append("\">")
                .append((i + 1) * value).append("dp</dimen>\r\n")
            if (i == 0) {
                if (currentSize / benchMarkSize == 1.0) {
                    stringBuilder.append("""<dimen name="dp1_5">1.5dp</dimen>""")
                } else {
                    stringBuilder.append("\t<dimen name=\"dp1_5" + "\">").append(value * 1.5)
                        .append("dp</dimen>\r\n")
                }
            } else {
                if (currentSize / benchMarkSize == 1.0) {
                    stringBuilder.append("\t<dimen name=\"dp").append(i + 1).append("_5")
        ...
        for (i in 6..50) {
            ...
            stringBuilder.append("\t<dimen name=\"sp").append(i).append("\">").append(value)
                .append("sp</dimen>\r\n")
        }
        stringBuilder.append("</resources>")
        //这里是文件名，1 注意修改 sw 后面的值，和转换值一一对应
        val fileDef = "./lib_base/src/main/res/values-w" + screenStr + "dp/dimens.xml"
        //文件写入流
